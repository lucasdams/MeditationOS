...
- [x] **Trataka (flame-gazing focus practice)** — an *eyes-open* concentration sit on a `/trataka` page: a procedurally-animated candle flame to gaze at (Canvas, no assets; respects `prefers-reduced-motion` with a still flame), a duration + background-safe timer reusing the meditation-timer flow, and an explanatory non-clinical "About" section. Logged under the existing `mindfulness` type — **no migration** — so it earns XP and feeds streaks/analytics. Surfaced in the "More" menu. See [ADR-0018](decisions/0018-trataka-flame-gazing-practice.md)
...
- [x] **Evolution trees — structure track** — the same framework (the `form` evolution fork + the shared `venerable` rung + a structure-appropriate additive slot, each with distinct SVG art) applied to every **structure** item: hut (thatched / treehouse / hermitage; window-box), cottage (cosy / grand-manor / enchanted; ivy), barn (working-farm / heritage / festival; weathervane), car (vintage / camper; flag), beach_house (cabana / lighthouse-keeper / stilt-house; bunting), boat (sailboat / fishing-boat; pennant). Reuses ADR-0021 (no new ADR); legacy `{"grown":"grown"}` preserved; no migration. Companion/whimsy tracks follow next.
- [x] **Evolution trees — companion track** — the same framework applied to every **companion**, where the forms read as personality / pose / markings: dog (playful / regal / guardian), cat (lap-cat / sleek-hunter / mystic), fox (woodland / arctic / fire-kissed), bird (songful / plumed / migratory), hedgehog (snug / forager), snake (coiled / patterned), goldfish (fantail / koi-kissed), snail (mossy-garden / jeweled). Each also gains the shared `venerable` rung (distinct 5th-stage art per animal) and one new additive **`toy`** slot (ball / yarn / stick / …) that doesn't duplicate the existing headwear / collar / attire dress-up slots. Reuses ADR-0021 (no new ADR); legacy `{"grown":"grown"}` preserved; no migration. Whimsy track follows next.
- [x] **Evolution trees — whimsy track (rollout COMPLETE)** — the same framework applied to every **whimsy** item, where the forms read as charming evolved characters: garden_gnome (wandering / wizardly / dozing), wind_chime (crystal-chime / pan-pipes), lantern (firefly / star / spirit), frog_lily (frog-prince / zen-frog), scarecrow (harvest-guard / spooky / dapper), fairy_door (mossy / royal / starlit), hammock (garden-swing / canopy), tea_cart (garden-party / patisserie / high-tea). Each also gains the shared `venerable` rung (distinct 5th-stage art per item) and one new themed additive slot (a toadstool / perched bird / charm / dragonfly / pumpkin-patch / welcome-mat / side-table / treats) that doesn't duplicate the item's own ADR-0016 slots. New form/option keys are namespaced to avoid clashing with existing global keys (e.g. `dozing` not the `sleepy` variant, `toadstool_cap` not the `toadstool` variant). **This is the final track — all four (nature, structure, companion, whimsy) now carry evolution trees** (a test asserts every catalog item has a `form` fork). Reuses ADR-0021 (no new ADR); legacy `{"grown":"grown"}` preserved; no migration.
...
- [x] **Dark mode** — Light / Dark / System toggle in Settings → Appearance, built on the CSS custom-properties theme system; `System` follows the OS `prefers-color-scheme`.
- [x] **Light + dark high-visibility pass** — a contrast/visibility sweep across **both** themes (borders, links, mood pills, toasts, nav tiles, Sanctuary canvas + plant art, heatmap/quest cells), plus a small accessibility pass (form-error association, skip targets, ARIA labels). CSS-level polish, no schema or ADR. Broader accessibility work remains open.
- [x] **Internationalization (i18n)** — SHIPPED: an in-house i18n layer (`frontend/src/i18n/`, no new dependency; i18next-compatible catalog shape) with per-domain **en + ja** catalogs, a `useT()`/`t()` API (interpolation + `Intl.PluralRules`), locale-aware `Intl` date/time/number formatting, a language picker in Settings → Appearance, `<html lang>` + a CJK font stack (`html[lang='ja']`), and client-persisted locale. All UI **chrome** is localized (every page, the auth flow, shared components, Settings, Paths). Deferred: **content** localization (guided-session scripts, daily readings, gratitude categories, backend emails, path names/blurbs) stays English for now; AI prompts/suggestions in Japanese; per-user server-side locale preference. See the Tier-L item below.
...
- [~] **i18n / localization (Tier L)** — PARTIALLY SHIPPED. Done: user-facing UI strings extracted into a message catalogue (`frontend/src/i18n/locales/{en,ja}/*` merged from per-domain modules), an in-house locale framework (`frontend/src/i18n/index.ts` — no external i18n dependency), a language selector (Settings → Appearance) with client-persisted locale, and locale-aware date/number/time formatting via `Intl`. Remaining/deferred: **per-user server-side** locale preference (currently `localStorage` only), localization of long-form **content** (readings, guided scripts, gratitude categories, backend transactional emails, catalog-driven path names/blurbs), AI prompt/suggestion localization, and **RTL** layout (Arabic/Hebrew) support in the CSS. English remains the source of truth (frozen: catalog EN values are byte-identical to the original literals, enforced by the test suite).
