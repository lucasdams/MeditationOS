...
> **personalized and rotating** (meditation 2 XP/min, breathing 3×, +5 gratitude, +5
> journal; each user picks ≥3 of meditate/breathe/gratitude/journal, and within each
> category the **specific quest rotates by the date** from a pool with **varied XP**
> — `app/services/quest_pool.py`, a pure `(category, date)` function) — exactly the
> "cheap to re-tune, no migration" benefit this ADR predicted; (2) the one new stored
> field is the user's quest **selection** (`users.quest_features`), a preference, not
> progress. See [gamification](../design/gamification.md) for current rules.
>
