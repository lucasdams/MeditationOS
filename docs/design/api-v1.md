...
`big_moments`, `spiritual`, `material`). **Suggestions** (8 per call) are generated
by Claude Haiku from the category alone (no user text is sent to the model) and fall
back to a randomized sample of a curated pool when `ANTHROPIC_API_KEY` is unset or
the call fails — so "show different ideas" stays fresh either way; see
