...
`category` is a fixed taxonomy (`people`, `health`, `nature`, `experiences`,
`growth`, `home`, `self`, `simple_pleasures`). **Suggestions** are generated by
Claude Haiku from the category alone (no user text is sent to the model) and fall
back to a curated set when `ANTHROPIC_API_KEY` is unset or the call fails — see
