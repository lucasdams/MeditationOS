...
- ✅ Sanctuary — a garden built through a small **spend economy**: earn **coins** as you level up, then **buy** procedural **vector (SVG)** items across **nature, structures, companions & whimsy** tracks, choose **variants** at purchase, and add mix-and-match **customizations** over time — including a **multi-stage growth ladder** (grown → flourishing → mature → ancient → venerable), a late-game **evolution fork** (a mutually-exclusive `form` slot — e.g. an oak → mighty / blossoming / hollow elder), and **dress-up slots** (headwear, collar, attire) on the characters; arrange items on a **movable grid**; add optional **name plaques, notes, and favourites**. A **progressive pricing** surcharge keeps the economy paced. Only the holdings are stored; the coin balance is computed. See [Sanctuary design](docs/design/sanctuary.md) and [ADR-0011–0021](docs/decisions/)
