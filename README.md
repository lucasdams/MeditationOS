...
- ✅ Stats dashboard — total practice time, current/longest streak, a weekly breakdown, a daily-goal ring, and a GitHub-style consistency heatmap (showing the **last ~12 weeks**, bucketed on the user's **local day**, per-user timezone)
...
- ✅ Levels & XP + **personalized, rotating daily quests** — earn XP from practice (meditation **2×**/min, breathing **3×**, plus gratitude & journal entries), level up; each user picks **≥3** of meditate/breathe/gratitude/journal, and within each category the **specific quest rotates daily** from a pool with **varied XP** (e.g. "Sit 10+ minutes" `+30`, "Breathe slow, ≤5 bpm" `+35`), with **itemized XP rewards**, a **front-loaded per-session curve**, a streak bonus, and a live reset countdown; level displayed as a neutral **level badge** (◆ + number) with coins and next unlock
...
- ✅ Seasonal + day/night theme — a subtle ambient background that shifts by **season** (auto by date, or pick one in **Settings → Appearance**) and **day phase** (dawn / day / dusk / night, from your local clock)
- ✅ Spirit — a single **living companion** you grow by practicing, front-and-centre on the home screen: it evolves down a **practice-shaped path** (your dominant practice picks a mini-Buddha, a wind spirit, or a bloom spirit) through **five stages** (spark → wisp → fledgling → ascendant → radiant), and it **reacts to you** — idle motion, a daily glow that brightens with practice, and an aura that **breathes in sync with the resonance pacer**. A dedicated **`/spirit`** page lets you spend coins on **cosmetics** and revisit a **collection of past spirits**. Progress is monotonic and never punishing; only the irreducible choices are stored, the rest is computed. See [Spirit design](docs/design/spirit.md)
...
| [Spirit](docs/design/spirit.md) | The companion that replaced the Sanctuary garden ([ADR-0022](docs/decisions/0022-spirit-companion-replaces-sanctuary.md)) — one creature per user; coin balance computed on read. UI currently dormant |
...
| [0012](docs/decisions/0012-sanctuary-personalization.md) | Sanctuary — variants + mix-and-match customizations |
| [0013](docs/decisions/0013-sanctuary-progressive-pricing.md) | Sanctuary — progressive pricing + economy retune |
| [0014](docs/decisions/0014-sanctuary-grid-layout.md) | Sanctuary — movable grid layout (cell separate from position) |
