...
def get_or_create_active_spirit(db: DBSession, user_id: uuid.UUID) -> Spirit:
    ...
    try:
        ...
        if spirit is None:
            # The unique index fired but no active spirit is visible — an unexpected DB
            # state, not a normal race. Surface it clearly rather than as a bare lookup error.
            raise RuntimeError(f"active spirit missing after IntegrityError for user {user_id}")
        return spirit
    db.refresh(spirit)
    return spirit
